...
export default function VideoUploader({ trailer }: VideoUploaderProps) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const response = await fetch(
      `/api/admin/create-upload?email=${currentUser?.email}`,
      {
        method: "POST",
    ...
    <div className="w-full">
      <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-gray-600 rounded-lg cursor-pointer bg-[#1a1a1a] hover:bg-[#222] transition">
        <div className="flex flex-col items-center justify-center pt-5 pb-6">
          ...
          <p className="text-xs text-gray-600 mt-1">MP4, MOV supported</p>
        </div>
        <input
          type="file"
          accept="video/*"
